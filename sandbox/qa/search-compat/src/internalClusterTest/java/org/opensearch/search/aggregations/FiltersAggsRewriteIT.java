...
public class FiltersAggsRewriteIT extends OpenSearchSingleNodeTestCase {
    ...
    public void testWrapperQueryIsRewritten() throws IOException {
        createIndexWithSimpleMappings("test", Settings.builder()
            .put("index.pluggable.dataformat.enabled", true)
            .put("index.pluggable.dataformat", "composite")
            .put("index.composite.primary_data_format", "parquet")
            .put("index.composite.secondary_data_formats", "lucene")
            .build(), "title", "type=text");
        client().prepareIndex("test").setSource("title", "foo bar baz").get();
        client().prepareIndex("test").setSource("title", "foo foo foo").get();
        client().prepareIndex("test").setSource("title", "bar baz bax").get();
    ...
    protected java.util.Collection<Class<? extends org.opensearch.plugins.Plugin>> getPlugins() {
        java.util.List<Class<? extends org.opensearch.plugins.Plugin>> _mustangPlugins = new java.util.ArrayList<>(super.getPlugins());
