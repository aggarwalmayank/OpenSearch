...
public class DeletePitMultiNodeIT extends OpenSearchIntegTestCase {
    ...
    public void setupIndex() throws ExecutionException, InterruptedException {
        ...
        client().prepareIndex("index").setSource("field", "value").setRefreshPolicy(IMMEDIATE).execute().get();
        ensureGreen();
    }
    ...
    public void testDeleteAllPits() throws Exception {
        ...
        client().prepareIndex("index1").setSource("field", "value").setRefreshPolicy(IMMEDIATE).execute().get();
        ensureGreen();
