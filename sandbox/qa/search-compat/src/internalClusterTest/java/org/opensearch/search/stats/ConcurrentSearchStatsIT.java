...
public class ConcurrentSearchStatsIT extends OpenSearchIntegTestCase {
    ...
    public void testConcurrentQueryCount() throws Exception {
        ...
        indexRandom(
            false,
            true,
            client().prepareIndex(INDEX_1).setSource("foo", "bar"),
            client().prepareIndex(INDEX_1).setSource("foo", "baz"),
            client().prepareIndex(INDEX_2).setSource("foo", "bar"),
            client().prepareIndex(INDEX_2).setSource("foo", "baz")
        );
    ...
    public void testAvgConcurrencyNodeLevel() throws InterruptedException {
        ...
        ensureGreen();
        for (int i = 0; i < 4; i++) {
            client().prepareIndex(INDEX_1).setSource("field", "value" + i).get();
            refresh();
        }
        ...
        ensureGreen();
        for (int i = 0; i < 4; i++) {
            client().prepareIndex(INDEX_2).setSource("field", "value" + i).get();
            refresh();
        }
    ...
    public void testAvgConcurrencyIndexLevel() throws InterruptedException {
        ...
        for (int i = 0; i < 4; i++) {
            client().prepareIndex(INDEX).setSource("field", "value" + i).setRouting("0").get();
            refresh();
        }
        for (int i = 4; i < 8; i++) {
            client().prepareIndex(INDEX).setSource("field", "value" + i).setRouting("1").get();
            refresh();
        }
    ...
    public void testThreadPoolWaitTime() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            client().prepareIndex(INDEX).setSource("field", "value" + i).get();
            refresh();
        }
        client().admin()
            .cluster()
