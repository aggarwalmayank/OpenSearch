    ...
    public void testMustNotRangeRewriteWithMissingValues() throws Exception {
        ...
        for (int i = 0; i < numDocs; i++) {
            ...
            if (i >= numDocsMissingIntValues) {
                client().prepareIndex("test").setSource(intField, i, termField, termValue).get();
            } else {
                client().prepareIndex("test").setSource(termField, termValue).get();
            }
        }
        ensureGreen();
        waitForRelocation();
    ...
    public void testMustNotRangeRewriteWithMoreThanOneValue() throws Exception {
        ...
        for (int i = 0; i < numDocs; i++) {
            if (i < numDocsWithExtraValue) {
                client().prepareIndex("test").setSource(intField, new int[] { extraValue, i }).get();
            } else {
                client().prepareIndex("test").setSource(intField, i).get();
            }
        }
        ensureGreen();
        waitForRelocation();
