...
public class SignificantTermsSignificanceScoreIT extends ParameterizedStaticSettingsOpenSearchIntegTestCase {
    ...
    public void testPopularTermManyDeletedDocs() throws Exception {
        ...
        indexRequestBuilderList.add(client().prepareIndex(INDEX_NAME).setSource(TEXT_FIELD, cat1v1, CLASS_FIELD, "1"));
        indexRequestBuilderList.add(client().prepareIndex(INDEX_NAME).setSource(TEXT_FIELD, cat1v2, CLASS_FIELD, "1"));
        indexRequestBuilderList.add(client().prepareIndex(INDEX_NAME).setSource(TEXT_FIELD, cat2v1, CLASS_FIELD, "2"));
        indexRequestBuilderList.add(client().prepareIndex(INDEX_NAME).setSource(TEXT_FIELD, cat2v2, CLASS_FIELD, "2"));
        ...
        for (int i = 0; i < 50; i++) {
            ...
            indexRequestBuilderList.add(client().prepareIndex(INDEX_NAME).setSource(TEXT_FIELD, text, CLASS_FIELD, "1"));
        }
        indexRandom(true, false, indexRequestBuilderList);
    ...
    private void indexEqualTestData() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            indexRequestBuilders.add(client().prepareIndex("test").setSource("class", parts[0], "text", parts[1]));
        }
    ...
    public void testScriptCaching() throws Exception {
        ...
        indexRandom(true, false,
            client().prepareIndex("cache_test_idx").setSource("s", 1),
            client().prepareIndex("cache_test_idx").setSource("s", 2)
        );
