...
public class QueryStringIT extends ParameterizedStaticSettingsOpenSearchIntegTestCase {
    ...
    public void testBasicAllQuery() throws Exception {
        ...
        reqs.add(client().prepareIndex("test").setSource("f1", "foo bar baz"));
        reqs.add(client().prepareIndex("test").setSource("f2", "Bar"));
        reqs.add(client().prepareIndex("test").setSource("f3", "foo bar baz"));
    ...
    public void testWithDate() throws Exception {
        List<IndexRequestBuilder> reqs = new ArrayList<>();
        reqs.add(client().prepareIndex("test").setSource("f1", "foo", "f_date", "2015/09/02"));
        reqs.add(client().prepareIndex("test").setSource("f1", "bar", "f_date", "2015/09/01"));
        indexRandom(true, false, reqs);
    ...
    public void testWithLotsOfTypes() throws Exception {
        List<IndexRequestBuilder> reqs = new ArrayList<>();
        reqs.add(
            client().prepareIndex("test").setSource("f1", "foo", "f_date", "2015/09/02", "f_float", "1.7", "f_ip", "127.0.0.1")
        );
        reqs.add(
            client().prepareIndex("test").setSource("f1", "bar", "f_date", "2015/09/01", "f_float", "1.8", "f_ip", "127.0.0.2")
        );
        indexRandom(true, false, reqs);
    ...
    public void testDocWithAllTypes() throws Exception {
        ...
        reqs.add(client().prepareIndex("test").setSource(docBody, MediaTypeRegistry.JSON));
        indexRandom(true, false, reqs);
    ...
    public void testKeywordWithWhitespace() throws Exception {
        List<IndexRequestBuilder> reqs = new ArrayList<>();
        reqs.add(client().prepareIndex("test").setSource("f2", "Foo Bar"));
        reqs.add(client().prepareIndex("test").setSource("f1", "bar"));
        reqs.add(client().prepareIndex("test").setSource("f1", "foo bar"));
        indexRandom(true, false, reqs);
        indexRandomForConcurrentSearch("test");
    ...
    public void testRegexCaseInsensitivity() throws Exception {
        ...
        indexRequests.add(client().prepareIndex("messages").setSource("message", "message: this is a TLS handshake"));
        indexRequests.add(client().prepareIndex("messages").setSource("message", "message: this is a tcp handshake"));
    ...
    public void testAllFields() throws Exception {
        ...
        List<IndexRequestBuilder> reqs = new ArrayList<>();
        reqs.add(client().prepareIndex("test_1").setSource("f1", "foo", "f2", "eggplant"));
        indexRandom(true, false, reqs);
    ...
    public void testPhraseQueryOnFieldWithNoPositions() throws Exception {
        List<IndexRequestBuilder> reqs = new ArrayList<>();
        reqs.add(client().prepareIndex("test").setSource("f1", "foo bar", "f4", "eggplant parmesan"));
        reqs.add(client().prepareIndex("test").setSource("f1", "foo bar", "f4", "chicken parmesan"));
        indexRandom(true, false, reqs);
    ...
    public void testLimitOnExpandedFieldsButIgnoreUnmappedFields() throws Exception {
        ...
        client().prepareIndex("ignoreunmappedfields").setSource("field1", "foo bar baz").get();
        refresh();
    ...
    public void testLimitOnExpandedFields() throws Exception {
        ...
        client().prepareIndex("testindex").setSource("field_A0", "foo bar baz").get();
        refresh();
    ...
    public void testFieldAlias() throws Exception {
        List<IndexRequestBuilder> indexRequests = new ArrayList<>();
        indexRequests.add(client().prepareIndex("test").setSource("f3", "text", "f2", "one"));
        indexRequests.add(client().prepareIndex("test").setSource("f3", "value", "f2", "two"));
        indexRequests.add(client().prepareIndex("test").setSource("f3", "another value", "f2", "three"));
        indexRandom(true, false, indexRequests);
