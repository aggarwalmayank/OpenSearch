...
public class DateRangeIT extends ParameterizedStaticSettingsOpenSearchIntegTestCase {
    ...
    protected Collection<Class<? extends Plugin>> nodePlugins() {
        return java.util.Arrays.asList(DateScriptMocksPlugin.class,
    ...
    public void testRangeWithFormatStringValue() throws Exception {
        ...
        indexRandom(true, false,
            client().prepareIndex(indexName).setSource(jsonBuilder().startObject().field("date", "00:16:40").endObject()),
            client().prepareIndex(indexName).setSource(jsonBuilder().startObject().field("date", "00:33:20").endObject()),
            client().prepareIndex(indexName).setSource(jsonBuilder().startObject().field("date", "00:50:00").endObject())
        );
    ...
    public void testRangeWithFormatNumericValue() throws Exception {
        ...
        indexRandom(true, false,
            client().prepareIndex(indexName).setSource(jsonBuilder().startObject().field("date", 1002).endObject()),
            client().prepareIndex(indexName).setSource(jsonBuilder().startObject().field("date", 2000).endObject()),
            client().prepareIndex(indexName).setSource(jsonBuilder().startObject().field("date", 3008).endObject())
        );
