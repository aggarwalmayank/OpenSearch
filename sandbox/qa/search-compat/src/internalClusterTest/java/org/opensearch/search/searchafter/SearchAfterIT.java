...
public class SearchAfterIT extends ParameterizedStaticSettingsOpenSearchIntegTestCase {
    ...
    public void testsShouldFail() throws Exception {
        ...
        ensureGreen();
        indexRandom(true, false, client().prepareIndex("test").setSource("field1", 0, "field2", "toto"));
        {
    ...
    public void testPitWithSearchAfter() throws Exception {
        ...
        indexRandom(true, false,
            client().prepareIndex("test").setSource("field1", 0),
            client().prepareIndex("test").setSource("field1", 100, "field2", "toto"),
            client().prepareIndex("test").setSource("field1", 101),
            client().prepareIndex("test").setSource("field1", 99)
        );
         ...
         */
        indexRandom(true, false, client().prepareIndex("test").setSource("field1", 102));
        sr = client().prepareSearch()
            .addSort("field1", SortOrder.ASC)
            .setQuery(matchAllQuery())
            .searchAfter(new Object[] { 0 })
    ...
    private void assertSearchFromWithSortValues(String indexName, List<List> documents, int reqSize) throws Exception {
        int numFields = documents.get(0).size();
        {
            ...
            for (int i = 0; i < documents.size(); i++) {
                ...
                builder.endObject();
                requests.add(client().prepareIndex(INDEX_NAME).setSource(builder));
            }
