...
public class FetchProfilerIT extends OpenSearchIntegTestCase {
    ...
    public void testVersionPhaseProfile() throws Exception {
        ...
        IndexRequestBuilder[] docs = new IndexRequestBuilder[numDocs];
        for (int i = 0; i < numDocs; i++) {
            docs[i] = client().prepareIndex("test").setSource("field1", English.intToEnglish(i));
        }
        indexRandom(true, false, docs);
    ...
    public void testMatchedQueriesPhaseProfile() throws Exception {
        createIndex("test");
        client().prepareIndex("test").setSource("field1", "The quick brown fox").get();
        refresh();
    ...
    public void testHighlightPhaseProfile() throws Exception {
        createIndex("test");
        client().prepareIndex("test").setSource("field1", "The quick brown fox jumps over the lazy dog").get();
        refresh();
    ...
    public void testFetchScorePhaseProfile() throws Exception {
        createIndex("test");
        client().prepareIndex("test").setSource("field1", "The quick brown fox", "field2", 42).get();
        refresh();
